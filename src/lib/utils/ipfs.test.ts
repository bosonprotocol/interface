...
describe("getIpfsGatewayUrl", () => {
  test('should return the same URI when it starts with "https://lens.infura-ipfs.io/ipfs/"', () => {
    const uri = `https://lens.infura-ipfs.io/ipfs/${validIpfsHash}`;
    const result = getIpfsGatewayUrl(uri);
    expect(result).toBe(uri);
  });

  test("should return a valid IPFS gateway URL when the input is a valid CID with additional path segments", () => {
  ...
  test("should return the same URI when it is falsy", () => {
    const uri = "";
    const result = getIpfsGatewayUrl(uri);
    expect(result).toBe(uri);
  });

  test("should return a valid IPFS gateway URL when the input is a valid IPFS URL", () => {
  ...
  test("should return a valid IPFS gateway URL when the input is a valid IPFS URL with query parameters", () => {
  ...
  test("should return a sanitized URL when the input is not a valid CID or IPFS URL", () => {
    const uri = "https://example.com/image.png";
    const result = getIpfsGatewayUrl(uri);
    expect(result).toBe(sanitizeUrl(uri));
  });

  test("should return a valid IPFS gateway URL when the input is a valid IPFS URL with additional path segments", () => {
  ...
  test("should return a sanitized URL when the input is not a valid CID or IPFS URL with additional path segments", () => {
    const uri = "https://example.com/image.jpg?size=large";
    const result = getIpfsGatewayUrl(uri);
    expect(result).toBe(sanitizeUrl(uri));
  });

  test("should return a valid IPFS gateway URL when the input is a valid IPFS URL with additional path segments and query parameters", () => {
  ...
  test("should return a sanitized URL when given an invalid CID input", () => {
    const uri = "invalidCID";
    const result = getIpfsGatewayUrl(uri);
    expect(result).toBe(sanitizeUrl(uri));
  });

  test("should return the same URI if it's a valid ipfs.io url with path segments and query parameters", () => {
    const uri = `https://ipfs.io/ipfs/${validIpfsHash}/nft.png?img-format=auto`;
    const result = getIpfsGatewayUrl(uri);
    expect(result).toBe(uri);
  });

  test("should return a sanitized URL when the input URI contains query parameters", () => {
  ...
  test("should return a sanitized URL when the input URI is just the cid", () => {

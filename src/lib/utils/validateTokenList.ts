  ...
  switch (schema) {
    case ValidationSchema.LIST:
      validatorImport = await import(
        "lib/utils/__generated__/validateTokenList"
      );
      break;
    case ValidationSchema.TOKENS:
      validatorImport = await import("lib/utils/__generated__/validateTokens");
      break;
    default:
...
export async function validateTokens(json: TokenInfo[]): Promise<TokenInfo[]> {
  try {
    ...
    throw new Error(`Tokens failed validation: ${(error as Error).message}`);
  }
}

  ...
  return useMemo(() => {
    ...
    switch (sortMethod) {
      ...
      case TokenSortMethod.PERCENT_CHANGE:
        tokenArray = tokenArray.sort(
          (a, b) =>
            (b?.market?.pricePercentChange?.value ?? 0) -
            (a?.market?.pricePercentChange?.value ?? 0)
        );
        break;
      case TokenSortMethod.TOTAL_VALUE_LOCKED:
        tokenArray = tokenArray.sort(
          (a, b) =>
            (b?.market?.totalValueLocked?.value ?? 0) -
            (a?.market?.totalValueLocked?.value ?? 0)
        );
        break;
  ...
  return useMemo(() => {
    ...
    if (lowercaseFilterString) {
      returnTokens = returnTokens?.filter((token) => {
        ...
        const nameIncludesFilterString = token?.name
          ?.toLowerCase()
          .includes(lowercaseFilterString);
...
export type TopToken = NonNullable<
  NonNullable<TopTokens100Query>["topTokens"]
>[number];
...
export function useTopTokens(chain: Chain): UseTopTokensReturnValue {
  ...
  const sparklines = useMemo(() => {
    const unwrappedTokens =
      chainId &&
      sparklineQuery?.topTokens?.map((topToken) =>
        unwrapToken(chainId, topToken)
      );
    ...
    unwrappedTokens?.forEach(
      (current) =>
        current?.address &&
        (map[current.address] =
          current?.market?.priceHistory?.filter(isPricePoint))
    );
    return map;

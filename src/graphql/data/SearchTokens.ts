...
import { ARB, WRAPPED_NATIVE_CURRENCY } from "lib/constants/tokens";
...
export type SearchToken = NonNullable<
  NonNullable<SearchTokensQuery["searchTokens"]>[number]
>;

/* Returns the more relevant cross-chain token based on native status and search chain */
function dedupeCrosschainTokens(
  current: SearchToken,
  existing: SearchToken | undefined,
  searchChain: Chain
) {
  if (!existing) return current;
  invariant(
    current.project?.id === existing.project?.id,
    "Cannot dedupe tokens within different tokenProjects"
  );
  ...
  if (
    current.standard === "NATIVE" &&
    (existing.standard !== "NATIVE" || current.chain === searchChain)
  )
    return current;

  // Prefer tokens on the searched chain, otherwise prefer mainnet tokens
  if (
    current.chain === searchChain ||
    (existing.chain !== searchChain && current.chain === Chain.Ethereum)
  )
    return current;
...
function searchTokenSortFunction(
  ...
  if (a.standard === "NATIVE") {
    ...
    return (
      (b.market?.volume24H?.value ?? 0) - (a.market?.volume24H?.value ?? 0)
    );
}

...
import { toSlippagePercent } from "lib/utils/slippage";
...
function getClassicTradeDetails(
  ...
  return {
    ...
    routes: classicQuote
      ? computeRoutes(currencyIn, currencyOut, classicQuote.route)
      : undefined
  };
}
...
export async function transformRoutesToTrade(
  ...
  const { gasUseEstimateUSD, blockNumber, routes, gasUseEstimate } =

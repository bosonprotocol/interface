...
export default function TokenDetails({
  ...
  const { token: detailedToken, didFetchFromChain } = useRelevantToken(
    address,
    pageChainId,
    tokenQueryData
  );
    ...
    (update: Chain) => {
      ...
      if (bridgedAddress) {
        startTokenTransition(() =>
          navigate(
            getTokenDetailsURL({ address: bridgedAddress, chain: update })
          )
        );
      } else if (didFetchFromChain || detailedToken?.isNative) {
        startTokenTransition(() =>
          navigate(getTokenDetailsURL({ address, chain: update }))
        );
      }
    },
    [
  ...
  const handleCurrencyChange = useCallback(
    (tokens: Pick<SwapState, Field.INPUT | Field.OUTPUT>) => {
      ...
      const newDefaultTokenID =
        tokens[Field.OUTPUT]?.currencyId ?? tokens[Field.INPUT]?.currencyId;
      startTokenTransition(() =>
        navigate(
          getTokenDetailsURL({
            ...
            inputAddress:
              ...
              tokens[Field.INPUT] &&
              tokens[Field.INPUT]?.currencyId !== newDefaultTokenID
            ...
            <TokenInfoContainer data-testid="token-info-container">
              <TokenNameCell>
                ...
                <TokenTitle>
                  ...
                  <TokenSymbol>
                    {detailedToken.symbol ?? <Trans>Symbol not found</Trans>}
                  </TokenSymbol>
                </TokenTitle>
              </TokenNameCell>
              <TokenActions>
          ...
          <div style={{ pointerEvents: isBlockedToken ? "none" : "auto" }}>
            <Swap
              ...
              prefilledState={{
                ...
                [Field.OUTPUT]: {
                  currencyId: address === NATIVE_CHAIN_ID ? "ETH" : address
                }
              }}

...
import SwapBuyFiatButton, {
  MOONPAY_REGION_AVAILABILITY_ARTICLE
} from "./SwapBuyFiatButton";

jest.mock("../../state/application/hooks");
const mockUseFiatOnrampAvailability =
  useFiatOnrampAvailability as jest.MockedFunction<
    typeof useFiatOnrampAvailability
  >;
...
describe("SwapBuyFiatButton.tsx", () => {
  ...
  it("matches base snapshot", () => {
    mockUseFiatOnrampAvailability.mockImplementation(
      mockUseFiatOnRampsUnavailable
    );
  ...
  it("fiat on ramps available in region, account unconnected", async () => {
    ...
    expect(
      screen.queryByTestId("fiat-on-ramp-unavailable-tooltip")
  ...
  it("fiat on ramps available in region, account connected", async () => {
    ...
    expect(
      screen.queryByTestId("buy-fiat-flow-incomplete-indicator")
    ).not.toBeInTheDocument();
  });
  ...
  it("fiat on ramps unavailable in region", async () => {
    ...
    expect(
      await screen.findByTestId("fiat-on-ramp-unavailable-tooltip")
    ).toBeInTheDocument();
    expect(await screen.findByText(/Learn more/i)).toHaveAttribute(
      "href",
      MOONPAY_REGION_AVAILABILITY_ARTICLE
    );

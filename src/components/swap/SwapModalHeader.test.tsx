...
describe("SwapModalHeader.tsx", () => {
  ...
  it("renders ETH input token for an ETH input UniswapX swap", () => {
    ...
    expect(screen.getByTestId("INPUT-amount")).toHaveTextContent(
      `${formatCurrencyAmount(
        TEST_DUTCH_TRADE_ETH_INPUT.inputAmount,
        NumberType.TokenTx
      )} ${ETH_MAINNET.symbol}`
    );
    expect(screen.getByTestId("OUTPUT-amount")).toHaveTextContent(
      `${formatCurrencyAmount(
        TEST_DUTCH_TRADE_ETH_INPUT.outputAmount,
        NumberType.TokenTx
  ...
  it("test trade exact output, no recipient", () => {
    ...
    expect(asFragment()).toMatchSnapshot();
    expect(
      screen.getByText(/Input is estimated. You will sell at most/i)
    ).toBeInTheDocument();

    expect(screen.getByTestId("INPUT-amount")).toHaveTextContent(
      `${formatCurrencyAmount(
        TEST_TRADE_EXACT_OUTPUT.inputAmount,
        NumberType.TokenTx
      )} ${TEST_TRADE_EXACT_OUTPUT.inputAmount.currency.symbol ?? ""}`
    );
    expect(screen.getByTestId("OUTPUT-amount")).toHaveTextContent(
      `${formatCurrencyAmount(
        TEST_TRADE_EXACT_OUTPUT.outputAmount,
        NumberType.TokenTx

...
const Option = styled(Grid)<{ isActive: boolean }>`
  width: auto;
  cursor: pointer;
  padding: 6px 12px;
  text-align: center;
  ...
  background: ${({ isActive, theme }) =>
    isActive ? theme.backgroundInteractive : "transparent"};
  ...
  return (
    ...
    >
      ...
      {tooLow || tooHigh ? (
        <Grid gap="md">
          <Warning />
          <Typography color="accentWarning">
            {tooLow ? (
              <>
                Slippage below {MINIMUM_RECOMMENDED_SLIPPAGE.toFixed(2)}% may

...
export default function AdminAside() {
  const { [UrlParameters.adminPage]: adminPage } = useParams();

  const handleUrl = useCallback((path: string) => {
    return generatePath(AdminRoutes.Admin, {
      [UrlParameters.adminPage]: path
    });
  }, []);
      ...
      <ul>
        {Object.keys(pageTypes).map((key: string) => {
          const {
            ...
            url,
            icon: Icon
          } = pageTypes[key as keyof typeof pageTypes];
          const isActive = adminPage === url;
